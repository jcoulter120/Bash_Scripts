...
for fileNum in range(minFile,maxFile):
    ...
    print "\n\n\nFILE   :"+str(fileNum)+""
    File = open('Pythia_'+str(fileNum)+'_numEvents'+numEventString+'/Pythia_'+str(fileNum)+'_numEvents'+numEventString+'.log')
    ...
    for i in range(0,4000000):
      ...
      #          sys.stdout.write(line)
        ...
        if "PYSTAT" in line:
            print "found PYSTAT"
            #check that all the lines are there
            for c in range(0,11):
                line = File.readline()
                print line
            for z in range(0,1):
                ...
                #if not line[68:-2]:
                   # continue
                #if '*' or '=' or '0*' in line[68:-2]:
                try:
                    SUM_STATString+=str(float(line[68:-2]))+","
                except:
                    print "BROKEN FILE"
                    SUM_STATString+=str(0)+","
                #else:
                    #sys.stdout.write(line)
                    #print line[68:-2]
